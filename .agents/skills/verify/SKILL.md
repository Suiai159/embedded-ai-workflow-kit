...
1. 执行：`python .agents/skills/verify/verify.py --test-only --req <req_file>`
2. 内部流程：
...
**步骤 1：单次验证**
- 运行：`python .agents/skills/verify/verify.py --single-run --max-retries 0 --req <req_file> [--port <port>]`
