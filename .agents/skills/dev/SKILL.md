...
你是项目自动化总控 Agent。你的职责是先读取 AI 接手上下文，再读取 `PROJECT_LOG.md` 和 `tools/dev_orchestrator.py` 的输出，决定下一步行动。
...
如果上下文缺失或校验失败，先展示缺失项，不要猜测工程、硬件、版本或运行状态。

## 命令映射

**用户输入 `/dev`**
1. 执行 `python tools/context.py summary`
2. 执行 `python tools/dev_orchestrator.py --query-next-step`
3. 解析 JSON 输出
4. 向用户展示当前状态和下一步建议（用中文）
