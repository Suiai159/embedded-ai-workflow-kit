...
`App/`、`Service/`、`Driver/` 是当前模板工程声明的默认分层，不是 AI workflow 框架强制绑定的目录名。其他项目复用本 workflow 时，可以在 `.context/engineering.yaml` 和 `.workflow/project.yaml` 中声明不同架构。

- 换主机平台或构建工具：只改 `.workflow/project.yaml`、`tools/workflow.py` 或工具链 adapter
- 换 MCU/开发板：可以改 `Driver` 内部实现、HAL 绑定和硬件上下文，但不得破坏当前项目声明的依赖契约
- `Core/`、`Drivers/`、`.ioc` 和工具工程文件属于平台/厂商生成边界，不承载业务分层架构
- 只有明确的架构重构任务才允许改变当前项目声明的职责边界

### 目录边界

| 类别 | 目录/文件 | 规则 |
|------|-----------|------|
| 当前项目架构目录 | `App/`, `Service/`, `Driver/`, `Test/` | 当前模板工程使用这些层；复用到其他项目时可重新声明 |
...
换平台或工具链时，优先修改 `.workflow/project.yaml`、`tools/workflow.py` 和 adapter；不要因为工具链变化而移动当前项目声明的架构目录。
...
**4. 添加新模块检查清单**
- [ ] 确定所属层级（读取 `.context/engineering.yaml` 中声明的 architecture layers）
- [ ] 检查依赖方向（无反向依赖）
- [ ] 确认硬件资源未被占用
...
- ❌ **禁止**把当前项目声明的架构目录放进 `Core/` 内（Core/ 只放 CubeMX 生成代码）
